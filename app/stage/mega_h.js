...
scene.hears('Добавить канал', ctx => ctx.scene.enter('add_ero_channel'))
...
scene.hears('Редактировать', async ctx => {
    ...
    channel ? ctx.scene.enter('ero_edit_name') : ctx.reply('Упс, не известная ошибка или отсутвует канал!', keyboard(['< Назад']).oneTime().resize().extra())
})
...
scene.hears('Удалить канал', async ctx => {
    let channel = await E_Channel.findById(ctx.session.counter)
    await E_Channel.remove({ _id: ctx.session.counter })
